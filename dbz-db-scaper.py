...
def export_results(dataframe, kind, fname):
    print(f"\nExporting {kind} results...")
    current_timestamp = datetime.now().strftime('%Y_%m_%d')
    # Convert the dictionary to a DataFrame
    df = pd.DataFrame(dataframe)
    csv_file_name = f'{fname}_{current_timestamp}.csv'
    csv_file_path = os.path.abspath(csv_file_name)
    excel_file_name = f'{fname}_{current_timestamp}.xlsx'
    excel_file_path = os.path.abspath(excel_file_name)
    # Save the DataFrame as a CSV/Excel file
    df.to_csv(csv_file_path, index=False)
    df.to_excel(excel_file_path, index=False)
    print(f"\tDBZ Dokkan Battle csv file saved at: '{csv_file_path}'")
